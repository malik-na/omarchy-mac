...
-- Mac fork: this hardware names the lid switch "Apple SMC power/lid events"
-- (verified via `hyprctl devices`), not the generic "Lid Switch"; also save +
-- switch off the keyboard backlight on close and restore it on open.
o.bind("switch:on:Apple SMC power/lid events", nil, "brightnessctl -s --device=kbd_backlight set 0; omarchy-hw-external-monitors && omarchy-hyprland-monitor-internal off", { locked = true })
o.bind("switch:off:Apple SMC power/lid events", nil, "omarchy-hyprland-monitor-internal on; brightnessctl -r --device=kbd_backlight", { locked = true })
