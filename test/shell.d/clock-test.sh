...
# Mac fork: the notch design keeps the bar center empty and places the clock on
# the right, so assert the clock lives in the right section instead of center.
shell_json=$(cd "$ROOT" && jq -r '[.bar.layout.right[].id] | join(",")' config/omarchy/shell.json)
[[ $shell_json == *"omarchy.clock"* ]] || fail "default bar layout includes the clock widget" "right: $shell_json"
[[ $shell_json != *"omarchy.calendar"* ]] || fail "clock hosts the calendar instead of a second bar pill" "right: $shell_json"
