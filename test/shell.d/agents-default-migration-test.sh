...
# Starting from the shipped default minus the widget, the migration has to put
# the bar back exactly as shipped. Reading the expectation from the shipped
# config keeps this honest for any curated layout, including the Mac fork's
# right-aligned clock.
shipped_ids() {
  jq -c --arg section "$1" '[.bar.layout[$section][]? | if type == "object" then .id else . end]' "$ROOT/config/omarchy/shell.json"
}
