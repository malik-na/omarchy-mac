...
if [[ -f $RESUME_DROP_IN ]] && omarchy-cmd-present btrfs limine-mkinitcpio limine-update && grep -q 'resume_offset="$' "$RESUME_DROP_IN" && [[ -f $SWAP_FILE ]]; then
  ...
  if [[ -n $RESUME_OFFSET ]]; then
    ...
    [[ -f /etc/default/limine ]] && sudo sed -i "s/resume_offset=\"$/resume_offset=$RESUME_OFFSET\"/" /etc/default/limine
